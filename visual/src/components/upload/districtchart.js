...
class DistrictChart extends Component {
  ...
  download(data, year) {
    ...
    axios({
      method: "post",
      url: "https://dropouts.herokuapp.com/dis",
      responseType: "blob",
      data: {
        data: data,
  ...
  filterRegion(index, year) {
    ...
    const selectedRegion = this.props.data.filter(region => {
      return (
        ...
        (region.year === year ||
          region.year === "YEAR" ||
          region.year === parseInt(year))
      );
    });
    console.log(selectedRegion);

    const graph = selectedRegion.map(regions => {
      const result = _.pick(regions, [
        "council",
        "enrolment_total",
        "dropout_total"
      ]);
    ...
    var finalUser = _.map(reducedUsers).map((user, index) => {
      ...
      var percent =
        isNaN(dropoutSum) || isNaN(regionSum)
          ? "Percentage dropout"
          : (dropoutSum / regionSum) * 100;
  ...
  render() {
    ...
    const years = this.props.data.filter(year => year.year !== "YEAR");
    const yearList = years.map(year => year.year);
    const regions = this.props.data.filter(
      ...
      <div>
        <div id="select">
          ...
          >
            ...
            <button onClick={this.handleRegion} type="submit">
              Refresh
            </button>
            <button
              onClick={event => {
                event.preventDefault();
                this.download(this.state.data, year);
              }}
            >
              Download Chart
            </button>
